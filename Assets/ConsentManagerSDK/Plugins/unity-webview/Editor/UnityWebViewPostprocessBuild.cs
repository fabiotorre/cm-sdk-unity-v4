...
#endif
{
    ...
    public void OnPreprocessBuild(BuildTarget buildTarget, string path) {
        if (buildTarget == BuildTarget.Android) {
            var candidates = new [] {
                "Packages/net.gree.unity-webview/Assets/Plugins/Android",
                "Assets/Plugins/Android",
                "Assets/ConsentManagerSDK/Plugins/unity-webview/Android"
            };
            string dev = null;
            string rel = null;
            foreach (var candidate in candidates) {
                var devCandidate = Path.Combine(candidate, "WebViewPlugin-development.aar.tmpl");
                var relCandidate = Path.Combine(candidate, "WebViewPlugin-release.aar.tmpl");
                if (File.Exists(devCandidate) && File.Exists(relCandidate)) {
                    dev = devCandidate;
                    rel = relCandidate;
                    break;
                }
            }
            if (string.IsNullOrEmpty(dev) || string.IsNullOrEmpty(rel)) {
                throw new FileNotFoundException("unity-webview templates not found. Ensure WebViewPlugin .aar templates are present.");
            }
